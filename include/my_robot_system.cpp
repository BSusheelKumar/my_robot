...
#include "my_robot/my_robot_system.hpp"
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("RoBotSystemHardware"), "PID values not supplied, using defaults.");
  }
...
hardware_interface::CallbackReturn RoBotSystemHardware::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("RoBotSystemHardware"), "Configuring ...please wait...");
  if (comms_.connected())
  {
    comms_.disconnect();
  }
  comms_.connect(cfg_.device, cfg_.baud_rate, cfg_.timeout_ms);
  RCLCPP_INFO(rclcpp::get_logger("RoBotSystemHardware"), "Successfully configured!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn RoBotSystemHardware::on_cleanup(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("RoBotSystemHardware"), "Cleaning up ...please wait...");
  if (comms_.connected())
  {
    comms_.disconnect();
  }
  RCLCPP_INFO(rclcpp::get_logger("RoBotSystemHardware"), "Successfully cleaned up!");
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  wheel_b_l_.pos = wheel_l_.calc_enc_angle();
  wheel_b_l_.vel = (wheel_b_l_.pos - pos_prev) / delta_seconds;

  pos_prev = wheel_b_r_.pos;
  wheel_b_r_.pos = wheel_r_.calc_enc_angle();
  wheel_b_r_.vel = (wheel_b_r_.pos - pos_prev) / delta_seconds;

  return hardware_interface::return_type::OK;
}

hardware_interface::return_type my_robot ::RoBotSystemHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  if (!comms_.connected())
  {
...
PLUGINLIB_EXPORT_CLASS(
  my_robot::RoBotSystemHardware, hardware_interface::SystemInterface)