...
class MyNode(Node):
    ...
    def read_callback(self,msg):
        if msg.data == "reached_goal":
            ...
            # Start searching for fire
            self.start_fire_search()
        else:
            self.get_logger().info("Goal Not Reached Yet")
            self.reached_goal = False

    def start_fire_search(self):
        self.timer = self.create_timer(0.1, self.rotate_until_fire_detected)

    def rotate_until_fire_detected(self):
        # Rotate the robot to search for fire
        move = Twist()
        move.angular.z = 0.5  # Angular velocity for rotation
    ...
    def camera_callback(self, data):
        if self.reached_goal:
            ...
            if len(fire) > 0:
                ...
                # Fire detected, stop rotation
                self.stop_rotation()
                # Process fire detection further if needed
            else:
                self.target_x = None  # No fire detected, reset target x-coordinate
            self.image_width = img.shape[1]  # Get image width
        else:
            self.stop_rotation()  # Stop rotation if the goal is not reached
